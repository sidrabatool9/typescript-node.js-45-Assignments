// Favorite Fruit: Make a array of your favorite fruits, and then write a series of independent
//  if statements that check for certain fruits in your array.
// * Make a array of your three favorite fruits and call it favorite_fruits.
// * Write five if statements. Each should check whether a certain kind of fruit is in your array.
//  If the fruit is in your array, the if block should print a statement, such as You really like
//  bananas!

let favorite_fruits: string[]=["Mango", "Watermelon", "Pineapple"];
if(favorite_fruits.includes("Mango")){
    console.log("You really like Mango shake!")
};

if(favorite_fruits.includes("Strawberry")){
    console.log("You really like Strawberry shake!")
};

if(favorite_fruits.includes("Pineapple")){
    console.log("You really like pineapple cake!")
};

if(favorite_fruits.includes("Kivi")){
    console.log("You really like kivi!")
};

if(favorite_fruits.includes("Watermelon")){
    console.log("You really like Watermelon!")
};



